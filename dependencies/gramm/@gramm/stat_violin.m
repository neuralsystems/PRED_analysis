...
if obj.handle_graphics
    ...
    points=gobjects(1,length(uni_x));
    means=gobjects(1,length(uni_x));
    errorbars=gobjects(1,length(uni_x));
...
for ind_x=1:length(uni_x)
    ...
    if ~isempty(ysel)
        ...
        if params.points
            ...
            points(ind_x)=plot(xsel,ysel,'o','MarkerEdgeColor','none','MarkerSize',10,'MarkerFaceColor',face_color);
            set_alpha(points(ind_x),1,face_alpha);
        end
        
        %Draw mean & errorbar
        if params.mean
            mean_ysel = nanmean(ysel);
            means(ind_x)=plot([boxmid(ind_x)-(boxw/2) boxmid(ind_x)+(boxw/2)],[mean_ysel mean_ysel],'k-','LineWidth',2);
            ci=nanstd(ysel)./sqrt(sum(~isnan(ysel)));
            yci=bsxfun(@plus,mean_ysel,[-ci;ci]);
            errorbars(ind_x)=plot([boxmid(ind_x) boxmid(ind_x)],[yci(1) yci(2)],'k-','LineWidth',2);
        end
    end
...
if params.mean
    obj.results.stat_violin{obj.result_ind,1}.mean_handle=means;
    obj.results.stat_violin{obj.result_ind,1}.errorbar_handle=errorbars;
end
end
